...
def upload_file():
    ...
    if file2:
        ...
        if allowed_file(file2.filename):
            ...
            file2.seek(0)
            file2.save(os.path.join(app.config['UPLOADED'], filename))
    ...
    if file1:
        ...
        if allowed_file(file.filename):
            ...
            file.save(os.path.join(app.config['UPLOADED'], filename))
            file.seek(0)
...
def index():
    ...
    uploaded=os.listdir(app.config['UPLOADED'])
    slider = os.listdir(app.config['SLİDER'])
    return render_template('index.html',slider=slider ,filenames=filenames,hele=hele,sketch=sketch ,converted_file=converted_file,sketch_file=sketch_file,reque=reque,uploaded=uploaded)
