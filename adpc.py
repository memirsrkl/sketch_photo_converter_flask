...
def yenis():
    import matplotlib.pyplot as plt
    loaded_styled_generator = tf.keras.models.load_model('working/saved_model/photo_generator')
    img_height, img_width = 256, 256
    channels = 3
    dosya = open("findpros.txt","r",encoding="utf-8")
    isim=dosya.readline()
    ...
    list_ds = tf.data.Dataset.list_files('static/sketch/*.jpg', shuffle=True)
    ...
    plt.savefig("static/convertings/" + isim[0:16].strip() + ".jpg",format="jpg",transparent=True, bbox_inches='tight', pad_inches=0)
        ...
        with open(dosya, "w", encoding="utf-8") as d:
            for index, line in enumerate(oku):
                if index == satir:
                    ...
                    d.write(line)
    satir_sil("findpros.txt", 0)
    stop_python_files()
    return isim
def stop_python_files():
